...
/**
 * Batch complete tickets mutation
 */
export function useBatchCompleteTickets() {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: (ids: string[]) => ticketApi.batchCompleteTickets(ids),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ticketKeys.lists() })
    },
  })
...
export function useBatchDeleteTickets() {
  ...
  return useMutation({
    mutationFn: (ids: string[]) => ticketApi.batchDeleteTickets(ids),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ticketKeys.lists() })
    },
  })
